...
|| |
|| | As the program is now, you can press the button again to start over at 'no
operation'
|| #
||
*/
/**
        README
        This code might seem over compicated and over engineered but it is
   highly maintainable.
        The code uses the design pattern of finite state machines,
        also it incorporates hardware abstraction of both the TLC and the Button
   used.

        I have not tested this code on hardware, simply because I have no
   tlc5940 around
        This means that I do not know if the program works as intended [like the
   curvature of the fade, or the fade at all]
        I am sorry about that, but if you have to run the code by me more that
   two times I'll do it for free, full 'refund'
        If we do more project I'll be sure to get my hands on the actual
   hardware

        Adding additional features or steps/states is simple
         1) change the NUMBER_OF_SELECATBLE_STATES constant if you add a new
   selective state such as fade and blink/animate
         2) add a definition of the state
         3) add a new case to the CONTROL THE STATE section at line 89
         4) implement the Update function of the state [like fadeUpdate,
   circleMotionUpdate etc]
...
State flash = State(flashUpdate); // this state flashes the leds
                                  // FLASH_ITERATIONS times at
                                  // 1000/FLASH_INTERVAL
State circleMotion = State(circleMotionUpdate); // show the circular animation

/** the state machine controls which of the states get attention and execution
 * time */
FSM stateMachine = FSM(noop); // initialize state machine, start in state: noop

/** this is the control interface, and is used to change the state of the
 * program */
Button button = Button(
    BUTTON_PIN,
    PULLUP); // initialize the button (wire between BUTTON_PIN and ground)
...
void loop() {
  ...
  static byte buttonPresses =
      0; // only accessible from this function, value is kept between iterations

  if (button.uniquePress()) {
    // increment buttonPresses and constrain it to [0,
    // NUMBER_OF_SELECATBLE_STATES-1]
    ...
    */
    ...
    case 1:
      ...
      break; // first press
    case 2:
      stateMachine.transitionTo(flash);
      break; // second press
    }
  }
  // THIS LINE IS CRITICAL
  // do not remove the stateMachine.update() call, it is what makes this program
  // 'tick'
  stateMachine.update();
}
...
void noopUpdate() {
  // this function gets called as long as the user have not pressed any buttons
  // after startup
}
///[fade state:enter] the statemachine is just told to enter the fade state, we
///need to
void fadeEnter() {
  ...
  for (int i = 0; i < NUM_TLCS * 16; i++) {
    ...
    // tlc_addFade(channel, FULL_INTENSITY, 0, endMillis, endMillis +
    // FADE_DURATION); fade back down?
  }
}
///[fade state:update] we need to keep updating the tlc in order to see the
///effects
void fadeUpdate() { tlc_updateFades(); }
///[flash state:update] this state blocks loop until done and does an
...
void circleMotionUpdate() {
  static TLC_CHANNEL_TYPE channel = 0; // only this function can access the
                                       // channel variable, and the value is
                                       // kept between calls
